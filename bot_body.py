                        ...
                        if username != "":
                            ...
                            # Broadcaster
                                ...
                                if "!goaway" in message.lower():
                                    ...
                                    if len(message.split(" ")) == 2:
                                        ...
                                        deathcounter.create_death_counter_file(
                                            character, 0
                                        )
                                        Send_message(
                                            f"Death counter has been set to track deaths for: {character}"
                                        )
                                        death_counter = True
                                        ...
                                        Send_message(
                                            "You have not provided enough information to set the current death counter. "
                                    ...
                                    message
                                    in ["!rip", "!ded", "!udead", "!medic", "!man down"]
                                    and death_counter is True
                                ):
                                    death_message = [
                                        f"Uh oh, looks like Rhyle_ died... again",
                                    ...
                                    print(
                                        myTwitch.update_twitch(
                                            ClientID, Token, update_info
                                        )
                                    )
                                    ...
                                    try:
                                        ...
                                        Send_message(
                                            "Sorry, you did not supply a game name to check the ID of."
                                        )
                                        break
                                    gameid = myTwitch.get_games(
                                        ClientID, Token, game_name
                                    )
                                    print(
                                        f"You requested the ID of {game_name}: {gameid}"
                                    )
                                elif message[0:8].lower() == "!adduser":
                                    command, new_user, user_type = message.split(" ")
                                    c.execute(
                                        "insert into users values (:user , :status)",
                                    ...
                                    chatmessage = c.execute(
                                        "select action from commands where ex_command = ?",
                                        (chatmessage.strip(""),),
                                    ).fetchone()[0]
                                    Send_message(chatmessage, username)
                                    chatmessage = c.execute(
                                        "select action from commands where ex_command = '!calls'"
                                    ).fetchone()[0]
                                    Send_message(chatmessage, username)
                                    continue
                                    ...
                                    try:
                                        ...
                                        with open(
                                            r"F:\Google Drive\Stream Assets\Bean list.txt",
                                            "r",
                                        ) as bean_list:
                                    ...
                                    # conn.commit()
                                    c.execute(
                                        "update users set gchar = ?, wins = 0 where uname = ?",
                                        (reset_character, person_to_reset),
                                    )
                                    conn.commit()
                                    # c.execute(
                                    ...
                                    for user in c.execute(
                                        "select * from users where status = 'viewer'"
                                    ).fetchall():
                                        # print(user[0])
                            ...
                            if message[:3].lower() not in (
                                ...
                                if "!lurk" in message.lower():
                                    ...
                                    chatmessage = (
                                        f"It looks like {username} no longer thinks they can be a "
                                    ...
                                    try:
                                        ...
                                        if cxp < 100:
                                            chatmessage = (
                                                f"Sorry {username}, you do not have enough accrued exp to"
                                                f" change your race at the moment, please try again later {cxp}/100."
                                            )
                                        elif race.lower() not in [
                                            "human",
                                            "elf",
                                            "halfling",
                                            "dwarf",
                                            ...
                                            chatmessage = (
                                                f"Sorry {username}, you must choose one of the 4 standard WFRP"
                                                f" races: Human, Elf, Dwarf, Halfling. Please try again."
                                            )
                                        elif (
                                            race.lower() == "human"
                                            and change_char["race"] != "human"
                                        ):
                                    ...
                                    try:
                                        ...
                                        if c.execute(
                                            "select gchar from users where uname = ?",
                                            (username.lower(),),
                                        ).fetchone() != ("",):
                                            gchar_dict_to_sql = c.execute(
                                                "select gchar from users where uname = ?",
                                            ...
                                            (
                                                ...
                                                "select exp, crowns, MaxWounds, CurrentWounds from users where uname = ?",
                                                (username,),
                                            ).fetchone()
                                            # print(gchar_dict)
                                            article = "a "
                                            ...
                                            if gchar_dict["prof"] == "peasant":
                                                build_whisper = (
                                                    f"{username} {username} is {article}"
                                                ...
                                                build_whisper2 = (
                                                    f"{username} You can level up your skills with !levelup and then one of the 4 skills or "
                                            ...
                                            gchar = character_manager.chat_char(
                                                username
                                            )
                                            ...
                                            chatmessage = (
                                                f"{username} the {chat_race.capitalize()} has "
                                                f"entered the game."
                                            )
                                    ...
                                    chatmessage = (
                                        f"Hello {username}, this command is being worked on at the "
                                    ...
                                    conn.commit()
                                    chatmessage = (
                                        f"{username} has chosen to permanently kill off their "
                                    ...
                                    try:
                                        ...
                                        Send_message(
                                            f"Blast! {username} the proper command is !challenge >target< >risk amount<"
                                        )
                                        continue
                                    ...
                                    if target == username:
                                        ...
                                        chatmessage = (
                                            f"Sorry {username}, either you or {target} do not currently "
                                        ...
                                        chatmessage = (
                                            f"{username} attempting to wager more exp than you have is "
                                            f"not allowed. You may risk only the exp you've earned."
                                        )
                                    else:
                                        chatmessage = (
                                            f"hey @{target}, {username} has wagered {str(absolute_amount)} exp that they"
                                            f" can take you down.  If you want to accept the fight type !accept or you can !decline."
                                        )
                                        pvp[
                                            (f"{username.lower()}", f"{time.time()}")
                                    ...
                                    for challenger, victim in pvp.items():
                                        if victim[0] == username:
                                            ...
                                            Send_message(
                                                f"{str(chall['name']).capitalize()}, {str(vic['name']).capitalize()} has accepted your challenge.  Prepare for combat!"
                                            )
                                            time.sleep(1)
                                            victim_random = randint(2, 100)
                                            vic_roll = (
                                                vic["strength"] + victim_random
                                            ) - chall["toughness"]
                                            if vic_roll < 0:
                                                vic_roll = 0
                                            Send_message(
                                                f"{victim[0]} hits {challenger[0]} with their {vic['weapon']} (({vic['weapon_skill']} + {victim_random})-{chall['toughness']}) ({vic_roll})"
                                            )
                                            time.sleep(1)
                                            challenger_random = randint(2, 100)
                                            chall_roll = (
                                                chall["strength"] + challenger_random
                                            ) - vic["toughness"]
                                            if chall_roll < 0:
                                                chall_roll = 0
                                            Send_message(
                                                f"{challenger[0]} returns the blow! (({chall['weapon_skill']} + {challenger_random}) - {vic['toughness']}) ({chall_roll})"
                                            )
                                            time.sleep(1)

                                            if vic_roll > chall_roll:
                                                Send_message(
                                                    f"{victim[0]} has defeated their challenger {challenger[0]} and earned! {amount} exp."
                                                )
                                                challenge_result(
                                                ...
                                                Send_message(
                                                    f"After a knock-down, drag-out fight both {challenger[0]}, and {victim[0]} have been ejected from the bar, {challenger[0]} clearly the victor, earning themselves {amount}."
                                                )
                                    ...
                                    chatmessage = (
                                        "This command will allow you to challenge another viewer with a "
                                    ...
                                    time_started = myTwitch.get_uptime(
                                        ClientID, oauth, Token
                                    )
                                    time_started = time_started["data"][0]["started_at"]
                                    time_started = datetime.datetime.strptime(
                                        time_started, "%Y-%m-%dT%H:%M:%SZ"
                                    )
                                    ...
                                    if len(message) <= 9:
                                        chatmessage = (
                                            f"/w {username} The proper command for this includes one of the "
                                ...
                                elif "!beaz" in message.lower().strip("\r") and (
                                    username.lower() == "big_beaz"
                                    or username.lower() == "rhyle_"
                                ):
                                    soundcommands.playme("beaz")
                                    chatmessage = ""

                                elif "!penny" in message.lower().strip("\r") and (
                                    username.lower() == "pennylane69739"
                                    or username.lower() == "rhyle_"
                                ):
                                    ...
                                    "!faerie" in message.lower().strip("\r")
                                    and username.lower() == "13thfaerie"
                                ):
                            ...
                            # Gunter command
                                ...
                                message.lower() == "!gunter"
                                or message.lower() == "!cmd"
                            ):
                                commandlist = list(
